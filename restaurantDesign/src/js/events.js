...
export function calculateBill() {
    ...
    cartItems.forEach((item) => {
        ...
        const price = parseFloat(document.querySelector(`#item-${index} #price-${index}`).textContent.replace("$ ", "")); // so from "$ 12" we get 12
        // console.log(price)
        const quantity = parseInt(document.querySelector(`#count-${index}`).textContent, 10);

        subTotal += price * quantity;
        gst += (5 / 100) * price * quantity;
    });
